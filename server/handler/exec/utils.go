...
func getFileDependencies(reqId string, procs []Process, files []File) ([][]sandbox.File, error) {
	fileMap := make(map[string]File)
	for _, file := range files {
		fileMap[file.Name] = file
	}

	fileDeps := make([][]sandbox.File, len(procs))
	lastOcc := make(map[string]int)

	for i, proc := range procs {
		procDir := getProcFileDir(reqId, i)
		fileDeps[i] = make([]sandbox.File, 0)
		...
		for _, fileName := range proc.Files {
			if _, exists := lastOcc[fileName]; !exists {
				...
				fileDeps[i] = append(fileDeps[i], sandbox.File{
...
func createSandboxConfig(reqId string, proc Process, fileDeps []sandbox.File) *sandbox.Config {
	return nil
}
